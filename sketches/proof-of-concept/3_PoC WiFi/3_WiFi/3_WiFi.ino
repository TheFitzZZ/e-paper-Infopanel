...
char* fp_api_github_com = "35 85 74 EF 67 35 A7 CE 40 69 50 F3 C0 F6 80 CF 80 3B 2E 19";
char* fp_github_com     = "ca 06 f5 6b 25 8b 7a 0d 4f 2b 05 47 09 39 47 86 51 15 19 84";
#if USE_BearSSL
char fp_rawcontent[20]  = {0xcc, 0xaa, 0x48, 0x48, 0x66, 0x46, 0x0e, 0x91, 0x53, 0x2c, 0x9c, 0x7c, 0x23, 0x2a, 0xb1, 0x74, 0x4d, 0x29, 0x9d, 0x33};
#else
char* fp_rawcontent     = "cc aa 48 48 66 46 0e 91 53 2c 9c 7c 23 2a b1 74 4d 29 9d 33";
#endif
...
void showBitmapFrom_HTTPS(char* host, char* path, char* filename, char* fingerprint, int16_t x, int16_t y, bool with_color = true);

void showBitmapFrom_HTTP_Buffered(char* host, char* path, char* filename, int16_t x, int16_t y, bool with_color = true);
void showBitmapFrom_HTTPS_Buffered(char* host, char* path, char* filename, char* fingerprint, int16_t x, int16_t y, bool with_color = true);
...
void drawBitmaps_fitzzz()
{
  ...
  showBitmapFrom_HTTPS(host_rawcontent, path_rawcontent2, "Test-6.bmp", fp_rawcontent, 0, 0); //Bild oben links
  //delay(10000);
}
...
void showBitmapFrom_HTTP(char* host, char* path, char* filename, int16_t x, int16_t y, bool with_color)
{
  WiFiClient client;
    ...
    if ((planes == 1) && ((format == 0) || (format == 3))) // uncompressed is handled, 565 also
    {
      ...
      if (w <= max_row_width) // handle with direct drawing
      {
        ...
        for (uint16_t row = 0; row < h; row++, rowPosition += rowSize) // for each line
        {
          ...
          //Serial.print("Setting wrong shit 1");
          int16_t test = 0;

          display.writeImage(output_row_mono_buffer, output_row_color_buffer, x, yrow, w, 1, false, false, false); //lame
...
void drawBitmapFrom_HTTP_ToBuffer(char* host, char* path, char* filename, int16_t x, int16_t y, bool with_color)
{
  WiFiClient client;
  bool connection_ok = false;
  bool valid = false; // valid format to be handled
...
void showBitmapFrom_HTTP_Buffered(char* host, char* path, char* filename, int16_t x, int16_t y, bool with_color)
{
  Serial.println(); Serial.print("downloading file \""); Serial.print(filename);  Serial.println("\"");
...
void showBitmapFrom_HTTPS(char* host, char* path, char* filename, char* fingerprint, int16_t x, int16_t y, bool with_color)
{
    ...
    if ((planes == 1) && ((format == 0) || (format == 3))) // uncompressed is handled, 565 also
    {
      ...
      if (w <= max_row_width) // handle with direct drawing
      {
        ...
        for (uint16_t row = 0; row < h; row++, rowPosition += rowSize) // for each line
        {
          ...
          int16_t yrow = y + (flip ? h - row - 1 : row);
          //Serial.print("Setting wrong shit 2");
          int16_t test = 0;
          display.writeImage(output_row_mono_buffer, output_row_color_buffer, x, yrow, w, 1, false, false, false); //lame
          //display.writeImage(output_row_mono_buffer, output_row_color_buffer, x, yrow, w, 1);
        } // end line
...
void drawBitmapFrom_HTTPS_ToBuffer(char* host, char* path, char* filename, char* fingerprint, int16_t x, int16_t y, bool with_color)
{
  // Use WiFiClientSecure class to create TLS connection
#if USE_BearSSL
  BearSSL::WiFiClientSecure client;
...
void showBitmapFrom_HTTPS_Buffered(char* host, char* path, char* filename, char* fingerprint, int16_t x, int16_t y, bool with_color)
{
  Serial.println(); Serial.print("downloading file \""); Serial.print(filename);  Serial.println("\"");
  display.setFullWindow();
  display.firstPage();
